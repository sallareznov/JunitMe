...
class DeliveryFactoryImpl implements DeliveryFactory {
	private final static DeliveryFactoryImpl INSTANCE = new DeliveryFactoryImpl();

	static DeliveryFactoryImpl instance() {
		return INSTANCE;
	}

	private DeliveryFactoryImpl() {
	}

	public <M, L extends MessageListener<M>> Work createDelivery(
			final M message,
			final MessageEndpointFactory messageEndpointFactory,
			final LogWriterFactory logWriterFactory) {
		return new Delivery<M, L>(message, messageEndpointFactory,
				logWriterFactory);
	}
}
